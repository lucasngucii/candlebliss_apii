...
export class Updatecategory1743240045978 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."orders_status_enum" AS ENUM('Đơn hàng vừa được tạo', 'Đang chờ thanh toán', 'Thanh toán thất bại', 'Thanh toán thành công', 'Đang xử lý', 'Đang giao hàng', 'Đã đặt hàng', 'Hoàn thành', 'Đã huỷ', 'Đổi trả hàng', 'Trả hàng/hoàn tiền')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."orders_status_enum_old" AS ENUM('Đã đặt hàng', 'Đang xử lý', 'Đang giao hàng', 'Hoàn thành', 'Đã huỷ', 'Đổi trả hàng', 'Trả hàng/hoàn tiền')`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" ALTER COLUMN "status" DROP DEFAULT`,
    );
